...
public class ListDowntimeUseCaseTest {
    ...
    public void givenAValidQuery_whenCallsListDowntime_thenShouldReturnDowntime() {
        final var downtime = List.of(
                Downtime.newDowntime(SensorId.from("1"), null, LocalDate.now(), Instant.now(), Instant.now().plusSeconds(1)),
                Downtime.newDowntime(SensorId.from("2"), null, LocalDate.now(), Instant.now(), Instant.now().plusSeconds(1))
        );
        ...
        final var aQuery = new DowntimeSearchQuery(expectedPage, expectedPerPage, expectedTerms, expectedSort, expectedDirection);
    ...
    public void givenAValidQuery_whenHasNoResults_thenShouldReturnEmptyDowntime() {
        final var downtime = List.<Downtime>of();
